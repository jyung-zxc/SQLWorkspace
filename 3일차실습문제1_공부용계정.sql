...
SELECT *
FROM (SELECT EMP_NAME, ((EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(CASE 
WHEN SUBSTR(EMP_NO,8,1) IN (1,2) THEN 19 || SUBSTR(EMP_NO,1,6) 
WHEN SUBSTR(EMP_NO,8,1) IN (3,4) THEN 20 || SUBSTR(EMP_NO,1,6)
END))))"나이", DEPT_TITLE , JOB_NAME 
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID) 
LEFT JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
GROUP BY EMP_NO, EMP_NAME, DEPT_TITLE, JOB_NAME
ORDER BY 나이 ASC)
WHERE ROWNUM = 1;
...
WITH EMP_SALARY AS (
    SELECT SUM(SALARY) AS 급여합 , AVG(SALARY) AS 평균
    FROM EMPLOYEE 
    -- 방법2
--    UNION
