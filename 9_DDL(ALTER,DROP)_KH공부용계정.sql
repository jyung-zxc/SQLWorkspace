...
-- CNAME 컬럼을 새로 추가.
-- 컬럼추가 ADD : ADD 컬럼명 자료형 DEFALUT 기본값
ALTER TABLE DEPT_COPY ADD CNAME VARCHAR2(20);
-- LNAME 칼럼을 DEPT_COPY에 추가하고 기본값으로 '한국'부여
...
/*
    2) 제약조건 추가/ 삭제
     제약조건추가
     - NOT NULL : MODIFY 칼럼명 NOTNULL;
     - PRIMARY KEY/ FOREIGN KEY/UNIQUE/CHECK
       ADD 제약조건(칼럼명/조건식);
*/
-- DEPT_COPY 테이블의 DEPT_ID에 PRIMARY KEY 제약조건 추가
-- DEPT_TITLE에 UNIQUE 제약조건, LNAME칼럼에 NOT NULL 제약조건 추가
ALTER TABLE DEPT_COPY 
...
-- SYS_C007159 -> DCOPY_DN_UQ
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007159 TO DCOPY_DN_UQ;

-- 테이블명 변경 : ALTER TABLE 테이블명 TO 바꿀테이블명
...
-- 어딘가에서 창조되고 있는 무보테이블은 바로 삭제되지 않는다.
-- 이 부모테이블을 삭제하고 싶다면
-- 방법1) 자식 테이블을 삭제후 부모테이블을 삭제
DROP TABLE 자식;
DROP TABLE 부모;
-- 방법2) 부모ㅔ이블 삭제시 맞물린 외래키 제약조건도 함께 삭제하는 방식
